      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          <el-popover
            placement="right"
            width="200"
            @hide="updateSalaryCfg(scope.row.user.userUid)"
            :key="scope.row.id"
            trigger="click">
  ...
  export default{
    ...
    methods: {
      showUpdateView(data){
        this.loadSalaries();
        if (data.salary) {
          this.sid = data.salary.salaryUid;
          this.osid = data.salary.salaryUid;
        } else {
          this.sid = '';
          this.osid = '';
        }
      ...
      updateSalaryCfg(eid){
        ...
        this.putRequest("/salary/sobcfg/", {
          eid: eid, sid: this.sid
        }
          ).then(resp=> {
          if(resp&&resp.status==200) {
            var data = resp.data;
