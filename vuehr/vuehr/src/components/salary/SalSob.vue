  ...
  export default {
    ...
    methods: {
      ...
      handleEdit(index, row) {
        ...
        this.salary.medicalPer = row.medialPer;
      ...
      next(){
        ...
        if (this.index == 7) {
          ...
          if (this.salary.id) {//更新
            _this.tableLoading = true;
            let addSalary ={
              name : _this.salary.name,
              salaryUid: _this.salary.id,
              createDate: new Date(_this.salary.createDate),
              basicSalary: _this.salary.basicSalary,
              trafficSalary: _this.salary.trafficSalary,
              lunchSalary: _this.salary.lunchSalary,
              bonus: _this.salary.bonus,
              pensionBase: _this.salary.pensionBase,
              pensionPer: _this.salary.pensionPer,
              medicalBase: _this.salary.medicalBase,
              medialPer: _this.salary.medicalPer,
              accumulationFundBase: _this.salary.accumulationFundBase,
              accumulationFundPer: _this.salary.accumulationFundPer
            }

            this.putRequest("/salary/sob/salary", addSalary).then(resp=> {
              _this.tableLoading = false;
              if (resp && resp.status == 200) {
                var data = resp.data;
                _
            ...
            this.$prompt('请输入账套名称', '提示', {
              ...
              this.postRequest("/salary/sob/salary", addSalary).then(resp=> {
                if (resp && resp.status == 200) {
                  var data = resp.data;
