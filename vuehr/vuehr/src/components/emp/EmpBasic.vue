            ...
            <el-col :span="6">
              <div>
                <el-form-item label="民族:" prop="nationId">
                  <el-select v-model="emp.nationId" @change="selectGet" style="width: 150px" size="mini" placeholder="请选择民族">
                    <el-option
                      v-for="item in nations"
                      :key="item.nationUid"
                      :label="item.name"
      ...
      deleteManyEmps() {
        this.$confirm('此操作将删除[' + this.multipleSelection.length + ']条数据, 是否继续?', '提示', {
          ...
          for (var i = 0; i < this.multipleSelection.length; i++) {
            ids += this.multipleSelection[i].userUid + ",";
          }
          this.doDelete(ids);
        }).catch(() => {
        });
...
//            _this.emptyEmpData();
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.emp.id) {
              //更新
              this.tableLoading = true;
              this.emp.birthday = new Date(this.emp.birthday);
              this.emp.conversionTime = new Date(this.emp.conversionTime);
              this.emp.endContract = new Date(this.emp.endContract);
              this.emp.beginContract = new Date(this.emp.beginContract);
              this.emp.beginDate = new Date(this.emp.beginDate);
      ...
      handleNodeClick(data) {
        ...
        this.emp.departmentId = data.departmentUid;
      ...
      showEditEmpView(row) {
        ...
        this.emp.id = row.userUid
        this.emp.name = row.realName
        ...
        this.emp.idCard = row.idCard
        this.emp.nationId = row.nation.nationUid
        this.emp.nativePlace = row.nativePlace
        this.emp.politicId = row.politicsStatus.politicsStatusUId
        this.emp.email = row.mail
        this.emp.phone = row.phone
        this.emp.address = row.address
        this.emp.departmentId = row.oweDepartment.departmentUid;
        this.emp.departmentName = row.oweDepartment.name;
        this.emp.jobLevelId = row.jobLevel.jobLevelUid;
        this.emp.posId = row.position.positionUid;
       ...
       /* this.emp.beginContract = new Date(row.beginContract);*/
