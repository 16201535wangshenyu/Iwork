  ...
  <div>
    ...
    <div style="margin-top: 10px">
      <el-table
        ...
        <el-table-column
          :prop="state=='jobtitle'?'jobLevelUid':'positionUid'"
          label="编号"
          width="80"
          align="left">
        </el-table-column>
    ...
    methods: {
      updatePosNameExec(){
        ...
        if(this.state == 'position'){
          this.putRequest(this.state=='position'?"/system/basic/position":"/system/basic/joblevel",
            { name: this.updatePosName,
              positionUid: this.updatePosId,
        ...
        this.$confirm('删除' + this.multipleSelection.length + '条数据, 是否继续?', '提示', {
          ...
          multipleSelection.forEach(row=> {
            if(_this.state == 'position')
                ids = ids + row.positionUid + ',';
            else
                ids = ids + row.jobLevelUid + ','
          })
          _this.doDelete(ids);
        }).catch(() => {
          this.$message({
      ...
      handleEdit(index, row){
        ...
        this.updatePosId = row.positionUid;
        this.updateTitleLevel=row.titleLevel;
        this.dialogVisible = true;
        this.updateJobId = row.jobLevelUid;
      },
      handleDelete(index, row){
        ...
        this.$confirm('删除[' + row.name + '], 是否继续?', '提示', {
          ...
          if(this.state == 'position')
              _this.doDelete(row.positionUid);
          else
              _this.doDelete(row.jobLevelUid);
        }).catch(() => {
          this.$message({
            type: 'info',
            message: '已取消删除'
