<template>
  <div class="per-leave__wrap">
    <el-tabs type="border-card">
      <el-tab-pane label="请假/调休记录">
        <el-table
          ...
          <el-table-column
            ...
            label="标签"
            width="100"
            :filters="[{ text: '请假', value: '请假' }, { text: '调休', value: '调休' }]"
            :filter-method="filterTag"
            filter-placement="bottom-end">
            <template slot-scope="scope">
              <el-tag
                :type="scope.row.tag === '请假' ? 'primary' : 'success'"
      ...
      <el-tab-pane label="未审核列表">
        <div style="margin-top: 20px">
          <el-button @click="toggleSelection(true)">同意所选</el-button>
          <el-button @click="toggleSelection(false)">拒绝所选</el-button>
        </div>
      ...
      <el-tab-pane label="已审核列表">
        <div style="margin-top: 20px">
          <el-button @click="revokeSelection()">撤销所选</el-button>
        </div>
        <el-table
          ref="passApplicationTable"
          :data="passApplication"
          tooltip-effect="dark"
          style="width: 100%"
    ...
    methods: {
      ...
      revokeSelection() {
        ...
        if (this.multipleSelection2) {
          ...
          postRequest('/evaluat/leave/revoke', {
            list: list,
          })
            .then(response => {
              ...
              arr.reverse().forEach((value, index) => {
                ...
                console.log(index);
                console.log(i)
                console.log(this.passApplication.length)
                this.passApplication.splice(i, i + 1)
                console.log(this.passApplication.length)
              })
              this.$refs.passApplicationTable.clearSelection();
      ...
      toggleSelection(flag) {
        if (flag) {
          if (this.multipleSelection) {
            ...
            postRequest('/evaluat/leave/pass', {
              list: list,
            })
              .then(response => {
                console.log('response')
          ...
          if (this.multipleSelection) {
            ...
            postRequest('/evaluat/leave/deny', {
              list: list,
            })
              .then(response => {
                console.log('response')
